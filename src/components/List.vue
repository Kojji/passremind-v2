...
async function list(recount = false) {
  loading.value = true;
  ...
  store.dispatch('entries/toggleMark', {idToken, index})
    .then((markStatus)=>{
      store.dispatch('misc/activateNotification', {
        duration: 3,
        message: markStatus ? "Entry marked as favorite!" : "Entry unmarked as favorite!",
        type: "success"
      })
    }).catch(()=>{
      store.dispatch('misc/activateNotification', {
...
<template>
  <div class="grid grid-flow-col grid-rows-4 gap-3" style="height: 60vh">
    <div class="row-span-1 col-span-1 flex items-center justify-center">
      <div class="flex flex-col items-center">
        <span class="text-sm text-gray-700">
          Showing <span class="font-semibold text-gray-700">{{((query.perPage * (query.page - 1)) + 1)}}</span> to <span class="font-semibold text-gray-700">{{((query.perPage * query.page) > query.total ? query.total : (query.perPage * query.page))}}</span> of <span class="font-semibold text-gray-700">{{totalEntries}}</span> Entries
        </span>
        <div class="inline-flex mt-2 xs:mt-0">
            <button :class="query.prev ? 'px-4 py-2 text-sm font-medium text-zinc-700 bg-zinc-200 rounded-l hover:bg-zinc-500 hover:text-zinc-200' : 'px-4 py-2 text-sm font-medium text-white bg-zinc-100 rounded-l cursor-not-allowed'" @click="query.page--; list(false)">
              Prev
            </button>
            <button :class="query.next ? 'px-4 py-2 text-sm font-medium text-zinc-700 bg-zinc-200 border-0 border-l border-white rounded-r hover:bg-zinc-500 hover:text-zinc-200' : 'px-4 py-2 text-sm font-medium text-white bg-zinc-100 border-0 border-l border-white rounded-r cursor-not-allowed'" @click="query.page++; list(false)">
              Next
            </button>
        </div>
      </div>
    ...
    <div class="relative shadow-inner appearance-none border rounded leading-tight row-span-3 col-span-1">
      ...
      <div v-else class="p-2 grid grid-rows-2 sm:grid-cols-3 md:grid-cols-4 grid-cols-2 gap-2 auto-cols-max h-full">
        <div v-for="entry, index in entries" :id="entry.id" class="row-span-1 col-span-1 rounded p-2 flex flex-wrap content-start min-h-min" style="box-shadow: 0 4px 6px -1px rgba(234, 152, 37, 0.5), 0 2px 4px -1px rgba(234, 152, 37, 0.06);" @click="$emit('editEntry', entry)">
          ...
          <div class="w-full flex justify-between">
            ...
            <font-awesome-icon style="color: orangered;" class="cursor-pointer" icon="fa-regular fa-clone" @click.stop="copyText(entry.login, 'Login')"/>
          </div>
          <div class="w-full flex justify-between">
            <p>{{entry.password.replace(/./g, '*')}}</p>
            <font-awesome-icon style="color: orangered;" class="cursor-pointer" icon="fa-regular fa-clone" @click.stop="copyText(entry.password, 'Password')"/>
          </div>
        </div>
      </div>
    </div>
